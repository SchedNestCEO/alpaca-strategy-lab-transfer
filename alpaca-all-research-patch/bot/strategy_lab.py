...
REGIME_VARIANT_KEYS = ("ma_20_100_green", "ma_50_200_green", "donchian_55_green")
...
OVERLAY_POLICIES = {
    "ma_20_100_news": (True, False),
    "ma_20_100_congress": (False, True),
    "ma_20_100_news_congress": (True, True),
    "ma_50_200_news": (True, False),
    "ma_50_200_congress": (False, True),
    "ma_50_200_news_congress": (True, True),
    "donchian_55_news": (True, False),
    "donchian_55_congress": (False, True),
    "donchian_55_news_congress": (True, True),
}
OVERLAY_BASE_KEYS = {
    "ma_20_100_news": "ma_20_100",
    "ma_20_100_congress": "ma_20_100",
    "ma_20_100_news_congress": "ma_20_100",
    "ma_50_200_news": "ma_50_200",
    "ma_50_200_congress": "ma_50_200",
    "ma_50_200_news_congress": "ma_50_200",
    "donchian_55_news": "donchian_55",
    "donchian_55_congress": "donchian_55",
    "donchian_55_news_congress": "donchian_55",
}
OVERLAY_STRATEGY_KEYS = tuple(OVERLAY_POLICIES)
...
STRATEGY_LABELS = {
    ...
    "ma_20_100_green": "MA 20/100 GREEN-only",
    "ma_50_200_green": "MA 50/200 GREEN-only",
    "donchian_55_green": "Donchian 55 GREEN-only",
    "ma_20_100_news": "MA 20/100 + News Veto",
    "ma_20_100_congress": "MA 20/100 + Congress",
    "ma_20_100_news_congress": "MA 20/100 + News + Congress",
    "ma_50_200_news": "MA 50/200 + News Veto",
    "ma_50_200_congress": "MA 50/200 + Congress",
    "ma_50_200_news_congress": "MA 50/200 + News + Congress",
    "donchian_55_news": "Donchian 55 + News Veto",
    "donchian_55_congress": "Donchian 55 + Congress",
    "donchian_55_news_congress": "Donchian 55 + News + Congress",
...
COMPARISON_COLUMNS = [
    ...
    "leader_qualified_at_entry",
    "consensus_count",
    "news_vetoed_candidates",
    "news_missing_candidates",
    "average_congress_score",
]
...
def _green_only_wrapper(fn: Callable[[pd.DataFrame, str], StrategySignal | None]) -> Callable[[pd.DataFrame, str, pd.DataFrame | None], StrategySignal | None]:
    """Exploratory regime hypothesis: preserve the base signal, but reject YELLOW.

    This wrapper is research-only and intentionally changes no stop, sizing, exit,
    cooldown, drawdown, or execution rule. It exists to test the post-hoc
    hypothesis that ambiguous SPY regimes caused a material share of failed
...
def strategy_definitions() -> dict[str, StrategyDefinition]:
    definitions = {
        ...
        "ma_20_100_green": StrategyDefinition(
            "ma_20_100_green",
            STRATEGY_LABELS["ma_20_100_green"],
            _green_only_wrapper(ma_20_100_signal),
            _exit_wrapper(ma_20_exit),
        ),
        "ma_50_200_green": StrategyDefinition(
            "ma_50_200_green",
            STRATEGY_LABELS["ma_50_200_green"],
            _green_only_wrapper(ma_50_200_signal),
            _exit_wrapper(ma_50_exit),
        ),
        "donchian_55_green": StrategyDefinition(
            "donchian_55_green",
            STRATEGY_LABELS["donchian_55_green"],
            _green_only_wrapper(donchian_55_signal),
            _exit_wrapper(donchian_20_exit),
        ),
    ...
    for overlay_key, base_key in OVERLAY_BASE_KEYS.items():
        base = definitions[base_key]
        definitions[overlay_key] = StrategyDefinition(
            overlay_key,
            STRATEGY_LABELS[overlay_key],
            base.signal_fn,
            base.exit_fn,
            max_hold_bars=base.max_hold_bars,
            count_three_line_occurrences=base.count_three_line_occurrences,
        )
    return definitions
        ...
        self.trades.append(
            {
                ...
                "congress_score": position.metadata.get("congress_score", ""),
                "news_verdict": position.metadata.get("news_verdict", ""),
                "news_reason": position.metadata.get("news_reason", ""),
            }
        )
...
class HistoricalOverlaySimulator(StrategySimulator):
    """Research-only historical replay for the production-style news/congress overlays.

    News is a veto only. Congress is a small ranking bonus only. Neither can create a
    technical signal, change sizing/stops, bypass RED, or bypass drawdown/daily-loss gates.
    """
    ...
    def __init__(
        ...
        bars: dict[str, pd.DataFrame],
        *,
        congress_signals: list[CongressDisclosure] | None = None,
        news_assessments: list[HistoricalNewsAssessment] | None = None,
        regime_fn: Callable[..., RegimeAssessment] = market_regime,
        prepared_bars: bool = False,
        precomputed_regimes: dict[date, RegimeAssessment] | None = None,
    ):
        if definition.key not in OVERLAY_POLICIES:
            raise ValueError(f"Unknown historical overlay strategy {definition.key!r}")
        self.use_news, self.use_congress = OVERLAY_POLICIES[definition.key]
        self.congress_signals = list(congress_signals or [])
        self.news_by_key = news_assessment_map(news_assessments or [])
        self.news_vetoed_candidates = 0
        self.news_missing_candidates = 0
        self.congress_scores_seen: list[float] = []
        super().__init__(
            definition,
            settings,
            bars,
    ...
    def run(self) -> LabResult:
        ...
        result.summary.update(
            {
                "news_vetoed_candidates": self.news_vetoed_candidates if self.use_news else None,
                "news_missing_candidates": self.news_missing_candidates if self.use_news else None,
                "average_congress_score": (
                    sum(self.congress_scores_seen) / len(self.congress_scores_seen)
                    if self.use_congress and self.congress_scores_seen
                    else (0.0 if self.use_congress else None)
                ),
            }
        )
        return result
    ...
    def _schedule_entries(
        ...
        spy_history = _frame_through(self.bars.get("SPY"), current_date)
        candidates: list[tuple[float, str, StrategySignal, date, dict[str, float], dict[str, Any]]] = []
        for symbol in self.settings.watchlist:
            if symbol in existing_symbols or self._cooldown_active(symbol, current_date):
                continue
            frame = self.bars.get(symbol)
            ...
            if self.use_news:
                assessment = self.news_by_key.get((symbol, current_date))
                if assessment is None:
                    self.news_missing_candidates += 1
                    continue
                metadata["news_verdict"] = assessment.verdict
                metadata["news_reason"] = assessment.reason
                if assessment.verdict != "SAFE":
                    self.news_vetoed_candidates += 1
                    continue

            congress_score = 0.0
            if self.use_congress:
                congress_score = congress_score_at(
                    self.congress_signals,
                    symbol=symbol,
                    signal_date=current_date,
                )
                self.congress_scores_seen.append(congress_score)
                metadata["congress_score"] = congress_score

            entry_date = _next_symbol_date(frame, current_date, self.settings.end)
            if entry_date is None:
                continue
            features = self._features(history)
            adjusted_score = float(signal.score) + (congress_score / 100.0) * 5.0
            adjusted_signal = replace(
                signal,
                score=adjusted_score,
                metadata={**dict(signal.metadata), **metadata},
            )
            candidates.append((adjusted_score, symbol, adjusted_signal, entry_date, features, metadata))

        candidates.sort(key=lambda item: (-item[0], item[1]))
        risk_capital = self._risk_capital(current_date)
        scheduled_today = 0
        for _, symbol, signal, entry_date, features, metadata in candidates:
            if slots <= 0 or scheduled_today >= self.settings.max_new_trades_per_day:
                break
            available_cash = max(Decimal("0"), self.cash - self.reserved_cash)
            cap = position_notional_cap(
...
def _simulate_periods(
    ...
    for period_name, (period_start, period_end) in periods.items():
        ...
        elif key in OVERLAY_STRATEGY_KEYS:
            if definition is None:
                raise ValueError(f"Missing strategy definition for {key}")
            use_news, use_congress = OVERLAY_POLICIES[key]
            if use_news and news_assessments is None:
                raise ValueError(f"Historical overlay {key} requires --news-assessments")
            if use_congress and congress_signals is None:
                raise ValueError(f"Historical overlay {key} requires --congress-signals")
            settings = _settings_for_period(base_settings, start=period_start, end=period_end, watchlist=lab_watchlist)
            results[period_name] = HistoricalOverlaySimulator(
                definition,
                settings,
                bars,
...
def _comparison_row(key: str, summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "news_vetoed_candidates": summary.get("news_vetoed_candidates"),
        "news_missing_candidates": summary.get("news_missing_candidates"),
        "average_congress_score": summary.get("average_congress_score"),
    }
...
def run_comparison(
    ...
    *,
    ...
    congress_signals_path: str | None = None,
    news_assessments_path: str | None = None,
    ...
    congress_signals = load_congress_signals_csv(congress_signals_path) if congress_signals_path else None
    news_assessments = load_news_assessments_csv(news_assessments_path) if news_assessments_path else None
    ...
    strategies: dict[str, StrategyPeriodResults] = {}
    strategy_keys: list[str] = list(BASE_STRATEGY_KEYS)
    if include_experimental:
        strategy_keys.extend(REGIME_VARIANT_KEYS)
    if copy_signals is not None:
        strategy_keys.extend(COPY_STRATEGY_KEYS)
    for overlay_key, (needs_news, needs_congress) in OVERLAY_POLICIES.items():
        if needs_news and news_assessments is None:
            continue
        if needs_congress and congress_signals is None:
            continue
        strategy_keys.append(overlay_key)
...
def run_research_suite(
    ...
    *,
    ...
    holdout_start_text: str,
    capital: str,
    copy_signals_path: str,
    congress_signals_path: str,
    news_assessments_path: str,
    slippages_text: str = "0.001,0.0025,0.005",
) -> int:
    """Run every implemented research strategy in one descriptive screening command.

    This intentionally requires all three historical input datasets so the suite cannot
    silently claim to have tested copy/congress/news when one source is absent. The
    repeatedly inspected historical holdout is descriptive validation, not untouched OOS.
    Survivors still require the Robustness Lab and forward paper evidence.
    """
    slippages: list[str] = []
    for raw in slippages_text.split(","):
        raw = raw.strip()
        if not raw:
            continue
        value = Decimal(raw)
        if value < 0 or value >= 1:
            raise ValueError("research-suite slippages must be between 0 and 1")
        normalized = str(value)
        if normalized not in slippages:
            slippages.append(normalized)
    if not slippages:
        raise ValueError("research-suite requires at least one slippage")

    # Fail before fetching any bars if an input schema/path is invalid.
    load_copy_signals_csv(copy_signals_path)
    load_congress_signals_csv(congress_signals_path)
    load_news_assessments_csv(news_assessments_path)

    comparison_root = cfg.project_root / "backtests" / "comparisons"
    suite_root = cfg.project_root / "backtests" / "research_suite"
    suite_root.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    output_dir = suite_root / timestamp
    suffix = 1
    while output_dir.exists():
        output_dir = suite_root / f"{timestamp}-{suffix}"
        suffix += 1
    output_dir.mkdir(parents=True)
    ...
    for slip in slippages:
        before = set(comparison_root.iterdir()) if comparison_root.exists() else set()
        print(f"\n=== ALL-RESEARCH SCREEN @ {Decimal(slip):.2%} SLIPPAGE ===", flush=True)
        code = run_comparison(
            cfg,
            ...
            capital=capital,
            slippage=slip,
            holdout_start_text=holdout_start_text,
            copy_signals_path=copy_signals_path,
            congress_signals_path=congress_signals_path,
            news_assessments_path=news_assessments_path,
            include_experimental=True,
        )
        if code != 0:
            return code
        after = set(comparison_root.iterdir())
        created = sorted(after - before)
        if len(created) != 1:
            raise RuntimeError("Could not uniquely identify research-suite comparison output")
        comparison_dir = created[0]
        runs.append({"slippage": float(Decimal(slip)), "comparison_dir": str(comparison_dir)})
        with (comparison_dir / "comparison_holdout.csv").open("r", newline="", encoding="utf-8") as handle:
            for row in csv.DictReader(handle):
                aggregate.append({"slippage": float(Decimal(slip)), **row})

    _write_csv(
        output_dir / "suite_holdout_screen.csv",
        ["slippage", *COMPARISON_COLUMNS],
        aggregate,
    )
    manifest = {
        "research_only": True,
        "paper_execution_modified": False,
        "holdout_status": "descriptive_reused_validation_period_not_untouched_oos",
        "inputs": {
            "copy_signals": str(copy_signals_path),
            "congress_signals": str(congress_signals_path),
            "news_assessments": str(news_assessments_path),
        },
        "slippages": [float(Decimal(value)) for value in slippages],
        "runs": runs,
    }
    ...
    print("\nALL-RESEARCH SCREEN COMPLETE")
    print(f"Combined holdout screen: {output_dir / 'suite_holdout_screen.csv'}")
    print("No strategy was promoted and paper execution remains unchanged.")
    return 0
