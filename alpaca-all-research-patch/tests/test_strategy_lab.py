...
def test_green_only_wrapper_rejects_yellow_without_changing_green_signal():
    calls = []

    def base_signal(history, regime):
        calls.append(regime)
        return StrategySignal(Decimal("0.03"), score=7.0)

    wrapped = _green_only_wrapper(base_signal)
    history = frame_from_closes([100, 101])
    assert wrapped(history, "YELLOW", None) is None
    assert calls == []
    signal = wrapped(history, "GREEN", None)
    assert signal is not None
    assert signal.stop_pct == Decimal("0.03")
    assert signal.score == 7.0
    assert calls == ["GREEN"]


def test_green_only_hypothesis_variants_are_registered():
...
def test_compare_all_research_sources_runs_every_registered_family(monkeypatch, tmp_path):
    from types import SimpleNamespace
    ...
    copy_path.write_text(
        "leader_id,symbol,side,leader_time,leader_price,signal_time,follower_price,source_trade_id,verified\n"
        "leader,AAA,BUY,2025-02-18T14:00:00+00:00,110,2025-02-18T14:01:00+00:00,111,t1,true\n"
        "leader,AAA,SELL,2025-02-25T14:00:00+00:00,115,2025-02-25T14:01:00+00:00,114,t1,true\n"
    )
    congress_path = tmp_path / "congress.csv"
    congress_path.write_text(
        "member_id,symbol,side,transaction_time,disclosure_time,verified\n"
        "m1,AAA,PURCHASE,2024-11-01T15:00:00+00:00,2025-01-05T15:00:00+00:00,true\n"
    )
    news_path = tmp_path / "news.csv"
    rows = ["symbol,signal_date,assessment_time,verdict,reason"]
    for day in all_dates:
        if date(2025, 1, 2) <= day <= date(2025, 3, 31):
            rows.append(f"AAA,{day.isoformat()},{day.isoformat()}T20:00:00+00:00,SAFE,synthetic")
            rows.append(f"SPY,{day.isoformat()},{day.isoformat()}T20:00:00+00:00,SAFE,synthetic")
            rows.append(f"QQQ,{day.isoformat()},{day.isoformat()}T20:00:00+00:00,SAFE,synthetic")
    ...
    rc = lab.run_comparison(
        ...
        congress_signals_path=str(congress_path),
        news_assessments_path=str(news_path),
        include_experimental=True,
    )
    assert rc == 0
    root = next((tmp_path / "backtests" / "comparisons").iterdir())
    expected = set(lab.BASE_STRATEGY_KEYS) | set(lab.REGIME_VARIANT_KEYS) | set(lab.COPY_STRATEGY_KEYS) | set(lab.OVERLAY_STRATEGY_KEYS)
