...
`run.py robustness` is a research-only stress harness for the surviving technical candidates. It never creates a broker client and never submits orders.
...
The default run performs six full/holdout slippage assumptions (`0`, `0.10%`, `0.15%`, `0.20%`, `0.25%`, `0.50%`), 12- and 24-month rolling windows, complete calendar-year windows, exposure-matched SPY/QQQ benchmarks, and a parallel 0.10%-slippage shadow continuation diagnostic. The official 5% high-water halt is never changed; the shadow is a separate hypothetical simulator used only to see what would have happened after the official halt.
...
The default rolling start step is 12 months to keep the first pass practical on Replit. For denser overlapping windows, use `--window-step-months 6` after the default pass.

Outputs are written beneath `backtests/robustness/<timestamp>/`:

- `summary.csv` / `summary.json` — fixed eligibility gates and stability score
- `slippage_stress.csv` — full and holdout results across the six slippage assumptions
- `rolling_and_calendar_windows.csv` — calendar-year and rolling-window performance
- `shadow_continuation.csv` — official halt versus hypothetical continuation at 0.10% slippage
- `exposure_matched_benchmarks.csv` — 30% SPY, 30% QQQ, and 15%/15% SPY/QQQ with the rest held as cash

The eligibility gates are fixed in advance rather than optimized on the holdout. A strategy must survive the baseline full period, show positive holdout edge with adequate sample size, survive the 0.25% stress, remain non-negative at 0.50%, and pass rolling-window stability/halt-rate gates. Passing the research gate does **not** arm paper execution or change `DRY_RUN`.
